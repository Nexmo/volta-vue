<template>
  <div>
    <div v-if="!isEmpty" :id="id" :class="gridClass"
      :style="style">
      <slot name="rows"></slot>
    </div>
    ...
    <div v-if="isEmpty" class="Vlt-empty Vlt-empty--search">
      <div class="Vlt-empty__content">
        <slot name="empty-message"><p>{{ emptyMessage }}</p></slot>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    style() {
      return this.custom ? `grid-template-columns: repeat(${this.numberOfColumns}, 1fr); grid-gap: ${this.gap}px` : '';
    },
  },
